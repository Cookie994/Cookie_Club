...
<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){ 
        ...
        if(!empty($_POST['email'])){
            ...
            $stmt = $db->prepare("SELECT id FROM users WHERE email = ? AND accesslevel >= ?");
            $stmt->bind_param("si", $email, $access);
            $stmt->execute();
            $res = $stmt->get_result();
        ...
        if($id){//if there is an id in db set temp pass
            ...
            if($stmt->affected_rows == 1){//if affected row is 1, send email
                ...
                try {
                    ...
                    $mail->Body    = "Please, log in using this password $tempPass.<br>Then you can change your password in your profile page.";
